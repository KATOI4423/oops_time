...
unsafe extern "system" fn keyboard_proc(n_code: i32, w_param: WPARAM, l_param: LPARAM) -> LRESULT
{
	...
	if let Some(hook) = *hook_guard {
		return CallNextHookEx(hook.0, n_code, w_param, l_param);
	}
...
{
	unsafe {
		let module_handle: HMODULE = GetModuleHandleW(None).expect("Failed to get module handle");
		let hinstance : HINSTANCE = module_handle.into();
		let hook: HHOOK = SetWindowsHookExW(WH_KEYBOARD_LL, Some(keyboard_proc), hinstance, 0)
